...
std::vector<std::string> load_class_list()
{
    ...
    std::ifstream ifs("config_files/v4.txt"); // Fájl megnyitása
    std::string line;
...
int main(int argc, char **argv)
{
    ...
    if (!capture.isOpened())
    {
        std::cerr << "Hiba a videó fájl megnyitása közben:\n Biztos létezik ilyen?\n Lehet rossz a kódolás \n";
        return -1;
    }
    ...
    std::cout << "Az összes frame ami fel lett dolgozva: " << total_frames << "\n"; 
    video.release(); 
    outfile.close(); 
